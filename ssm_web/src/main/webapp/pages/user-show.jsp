								...
								<tbody>
									<c:forEach items="${user.roles}" var="role" varStatus="vs">
										<tr data-tt-id="${vs.index+1}" data-tt-parent-id="0">
											<td>${role.roleName }</td>
											<td>${role.roleDesc }</td>
											<td><a href="${pageContext.request.contextPath}/user/deleteUserRole.do?userId=${user.id}&roleId=${role.id}" class="btn bg-olive btn-xs">删除角色</a></td>
										</tr>
										<c:forEach items="${role.permissions}" var="permission">
											<tr data-tt-id="1-1" data-tt-parent-id="${vs.index+1}">
												<td>${permission.permissionName}</td>
												<td colspan="3">${permission.url}</td>
											</tr>
